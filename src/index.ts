...
app.post('/', async (c) => {
  ...
  const uploadableImages = (files ?? []).filter((f) => f.type === 'image/jpeg' || f.type === 'image/png'
        || f.type === 'image/webp');
  const uploadableVideos = (files ?? []).filter((f) => f.type === 'image/gif' || f.type === 'video/mp4');
  ...
  const mediaIds = await Promise.all(uploadFiles.map(async (file) => await uploadMediaFromURL(file.url, file.type, authToken, ct0)));

  const filesStr: string = [
    ...unuploadFiles,
    ...(files ?? []).filter((f) => !(f.type.includes('image') || f.type === 'video/mp4')),
  ].reduce((p: string, c) => {
